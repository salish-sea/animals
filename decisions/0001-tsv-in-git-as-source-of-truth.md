...
- `dist/` is committed so consumers can fetch a raw URL without a build step, and
  regenerated by CI. CI fails if the committed output differs from a fresh build.
