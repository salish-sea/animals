...
They are also the acceptance tests. Each one should eventually have a query in
`bin/` that answers it.
...
| C5 | Is this entity a Southern Resident? (at any depth below the ecotype) | Both, filtering | Yes, by walking `membership.tsv` |
| C6 | This bout is tagged with a matriline. Which pod and ecotype does that roll up to? | SalishSea.io ingest | Yes |
...
| Who is this animal's mother? | Implied by matriline membership; recording it separately duplicates a photo-ID catalogue's job. |
