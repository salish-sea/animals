...
- **`SSA:0000908` *Laridae* deliberately has no code**, and the `Gull` row now says so.
  ...
  it would narrow the entity through a name and would pass validation, because nothing
  else in the data contradicts it: the failure ADR-0019's guarantee is written against,
  arriving by a route that guarantee does not cover. Written down so it is not "fixed"
  later by someone noticing the gap.
- **One new source, `IBP`.** Unlike `ORCASOUND`, it is an authority: an alpha code is the
  code the Institute for Bird Populations assigns to a species, true whether or not a
  moderator ever types it. That is why these are `IBP` rows and not use-evidence rows.
  Not a departure from ADR-0019, which rejected variant-rows for *typographic* variation
  — `OSPR` does not fold to `osprey`; it is a name from a separate naming system that
  denotes the same animal, so a row is the right mechanism, and the burden is bounded at
  one per bird species.
